...
<head>
    <meta charset="utf-8">
    <c:if test="${difficulty.id != null}">
        <title>SkiSystem - panel administracyjny - edycja poziomu trudności ${difficulty.name}</title>
    </c:if>
    <c:if test="${difficulty.id == null}">
        <title>SkiSystem - panel administracyjny - dodawanie poziomu trudności</title>
    </c:if>
...
<div class="container py-5">
    <div class="row">
        <div class="col-lg-6 offset-lg-3">
            <a class="btn btn-lg btn-light float-left mr-3" href="<spring:url value="/admin/difficulties"/>"><i class="fas fa-arrow-left"></i></a>

            <c:if test="${difficulty.id != null}">
                <h1 id="header">Edycja poziomu trudności ${difficulty.name}</h1>
            </c:if>
            <c:if test="${difficulty.id == null}">
                <h1 id="header">Dodawanie poziomu trudności</h1>
            </c:if>

            <form:form method="post" class="mt-5" modelAttribute="difficulty" action="/admin/difficulties/submit">
                ...
                <div class="text-right">
                    <a href="<spring:url value="/admin/difficulties"/>" class="btn btn-light">Wróć</a>&ensp;

...
<head>
    <meta charset="utf-8">
    <title>SkiSystem - panel administracyjny - edycja wyciągów</title>
...
<div class="container py-5">
    ...
    <h1 id="header">Wyciągi</h1>
    <div id="help" class="mt-4" style="display:none">
        <p>Moduł umożliwia edycję wykazu wyciągów znajdującego się w serwisie. Wszystkie informacje podawane w polach są wyświetlane w serwisie publicznie i mają charakter czysto informacyjny. Deaktywacja wyciągu oznacza, że przestaje być widoczny w wykazie.</p>
        <h3>Pola</h3>
        <ul>
            <li>Nazwa</li>
        ...
        <p>Dni tygodnia, dla których definiuje się godziny otwarcia, można edytować w menu Konfiuracja <i class="fas fa-caret-right"></i> Dni tygodnia.</p>
    </div>
    <form action="<spring:url value="/admin/lifts/update"/>" method="GET" class="mt-5">
        ...
        <table class="table">
            ...
            <c:forEach items="${lifts}" var="lift">
                <tr>
                    ...
                    <td>${lift.name}</td>
                    <td>${lift.length} m</td>
                    <c:if test="${lift.getIsActive()}">
                        <td><a class="text-success" href="<spring:url value="/admin/lifts/update?ids%5B%5D=${lift.id}&action=deactivate"/>" title="Dezaktywuj"><i class="fas fa-check text-success"></i></a></td>
                    </c:if>
                    <c:if test="${!lift.getIsActive()}">
                        <td><a class="text-success" href="<spring:url value="/admin/lifts/update?ids%5B%5D=${lift.id}&action=activate"/>" title="Aktywuj"><i class="fas fa-times text-danger"></i></a></td>
                    </c:if>
                    <td class="text-right py-2">
                        <div class="btn-group">
                            <a class="btn btn-primary fas fa-pen" href="<spring:url value="/admin/lifts/edit/${lift.id}"/>"></a>
                            <a class="btn btn-light fas fa-trash-alt" href="<spring:url value="/admin/lifts/update?ids%5B%5D=${lift.id}&action=delete"/>"></a>
                        </div>
                    </td>
                </tr>
            </c:forEach>
        ...
        <div>
            <a href="<spring:url value="/admin/lifts/add"/>" class="btn btn-primary float-right">Dodaj stok&ensp;<i class="fas fa-plus"></i></a>
